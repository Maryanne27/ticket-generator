    ...
    <div className="bg-darkBlue p-4 sm:p-6 border border-bgBlue rounded-lg shadow-lg">
      ...
      {inputFields.map(({ name, label, type, icon, placeholder }, i) => (
        <div key={i} className="mt-4">
          <label className="text-[FAFAFA] text-xs sm:text-sm">{label}</label>
          <div className="relative flex items-center">
            {icon && !formData[name] && <span className="">{icon}</span>}
                ...
                className={`w-full p-2 sm:p-3 ${
                  name === "email"
                    ? formData[name]
                      ...
                      : "pl-12"
                    : "text-left"
                } bg-[#02191D] text-[FAFAFA] rounded-lg border focus:outline-none transition-all ${
                  errors[name] ? "border-red-500" : "border-[#24A0B5]/30"
                }`}
                value={formData[name]}
                onChange={handleChange}
...
<Button type="freeTicket" onClick={handleSubmit} isLoading={loading} />
