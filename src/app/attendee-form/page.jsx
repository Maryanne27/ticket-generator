...
export default function Page() {
  ...
  const inputFields = [
    { name: "name", label: "Enter your name", type: "text", icon: null },
    {
      ...
      icon: <EnvelopeIcon className=" text-[#FAFAFA] w-6 h-6" />,
    },
    {
      name: "request",
  ...
  const validateForm = () => {
    ...
    if (formData.request && formData.request.length < 15) {
      newErrors.request = "Request must be at least 15 characters";
    }
  ...
  return (
    <div className="relative z-10 p-4 sm:p-6 border mx-auto border-bgBlue rounded-lg shadow-lg w-[350px] md:w-[500px] lg:w-[700px] justify-center">
      ...
      <div className="bg-darkBlue p-4 sm:p-6 border border-bgBlue rounded-lg shadow-lg">
        <div className="border border-bgBlue rounded-lg px-4 bg-[#08252B] pb-10 pt-4">
          <p className="text-[#FAFAFA] pb-6 text-sm sm:text-base md:text-lg font-roboto font-normal text-center lg:text-left md:text-left">
              ...
              >
                {formData.uploadImage ? (
                  <img
                    ...
                    alt="Uploaded"
                    className="w-32 sm:w-40 md:w-56 h-32 sm:h-40 md:h-56 rounded-3xl object-cover border-4 border-[#24A0B5]/50"
                  />
                ) : (
        ...
        {inputFields.map(({ name, label, type, icon, placeholder }, i) => (
          <div key={i} className="mt-4">
            <label className="text-[FAFAFA] text-xs sm:text-sm mb-5">{label}</label>
            <div className="relative flex items-center">
              {icon && (
                <span className="absolute left-3 sm:left-3 md:left-3 lg:left-3">
                  {icon}
                </span>
              )}
              ...
              {type === "textarea" ? (
                ...
                <input
                  ...
                  className={`w-full p-2 sm:p-3 pl-10 sm:pl-10 md:pl-10 lg:pl-10 bg-[#02191D] text-[#FAFAFA] rounded-lg border focus:outline-none transition-all ${
                    errors[name] ? "border-red-500" : "border-[#24A0B5]/30"
                  }`}
                  value={formData[name]}
                  onChange={handleChange}
